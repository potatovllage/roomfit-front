...
const apiBaseUrl = (import.meta.env.VITE_ROOMFIT_API_BASE_URL ?? 'http://43.203.208.230:8000').replace(/\/$/, '')
...
interface ThemeListResponse { themes: Theme[] }
...
export interface FurnitureItem {
  ...
  placement: { center_x: number; center_y: number; width: number; height: number; rotation_degrees: number }
}

export interface DesignJob {
  design_id: string
  status: 'queued' | 'analyzing' | 'selecting_products' | 'rendering' | 'succeeded' | 'failed'
  progress: number
  poll_after_ms: number | null
  result: {
    summary: string
    rendered_image: { url: string; width: number; height: number; expires_at: string }
    budget: { estimated_furniture_total: number; requested_max_amount: number; within_budget: boolean }
...
async function request<T>(path: string, init?: RequestInit): Promise<T> {
  ...
  throw new Error(body?.error?.message ?? `API 요청에 실패했습니다. (${response.status})`)
}
...
export function useCreateDesignMutation() {
  return useMutation({
    mutationFn: async ({ image, theme, maxAmount, prompt }: CreateDesignInput) => {
